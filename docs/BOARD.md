...
| T22 | Английский по умолчанию в CLI и MCP: вывод, ошибки, подсказки, шаблон `/figma`, метаданные `tools/list` | `src/cli/**`, `src/mcp/**` | claude | 2026-09-23T06:26Z | done |
...
| T30 | Ограничить запись экспортёра пределами vault при симлинках и подготовленных путях | `src/pull/**`, `docs/BOARD.md`, `docs/LOG.md` | codex | 2026-09-24T12:30Z | wip |
| T31 | Security review, зона Claude: dev-сервер (localhost, границы пути, битый URL), недоверенный текст макета в `/figma` и instructions MCP, realpath в `vault_get_asset` | `scripts/serve.mjs`, `src/cli/init.ts`, `src/mcp/**`, `docs/BOARD.md`, `docs/LOG.md` | claude | 2026-09-24T12:33Z | wip |
