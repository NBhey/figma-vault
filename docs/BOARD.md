...
| T2 | Фикстура `vault/example/` — рукописный doc.json по контракту + 2 ассета | `vault/example/**` | claude | 2026-09-22T14:30Z | wip |
| T3 | Экспортёр: Figma REST → нормализация → запись в vault. CLI `pull <figma-url>` | `src/pull/**` | codex | 2026-09-22T14:44Z | done |
| T4 | MCP-сервер stdio: 6 инструментов из контракта, читает только vault | `src/mcp/**` | claude | 2026-09-22T14:30Z | wip |
| T5 | Валидатор doc.json против контракта, общий для обеих сторон | `src/shared/**` | codex | 2026-09-22T14:59Z | wip |
